...
function myDriver(opts,app) {
  ...
  app.on('client::up',function(){
    ...
    setInterval(function() { self.registerDevices();}, 60000);
...
myDriver.prototype.registerDevices = function(rpc,cb) {
  var self = this;
  var path = "/sys/bus/w1/devices/";
  if (fs.existsSync(path)) {
      var contents = fs.readdirSync(path);
      contents.forEach( function(fname, i, arr) {
        if (fname.indexOf('28-') == 0) {
            // Register a device
            var uid = fname.substr(3);

            if (!self.registeredDevices[uid]) {
                var device = new Device(uid);
                self.registeredDevices[uid] = device;
